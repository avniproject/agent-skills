...
```python
status = subprocess.check_call([sys.executable, 'setup.py', 'build_ext', f'--build-lib={tmpdir}', f'--build-temp={tmpdir}'], env=env, cwd=os.path.join(base_dir, 'mypyc', 'lib-rt'))
```
...
*Source: test_external.py:18 | Complexity: Intermediate | Last updated: 2026-02-20*