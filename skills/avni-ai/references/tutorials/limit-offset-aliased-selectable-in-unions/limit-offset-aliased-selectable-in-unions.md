# How To: Limit Offset Aliased Selectable In Unions
...
Workflow: test limit offset aliased selectable in unions
...
### Step 2: Assign s1 = select.where.limit.order_by.alias.select(...)

```python
s1 = select(table).where(table.c.id == 2).limit(1).order_by(table.c.id).alias().select()
```

### Step 3: Assign s2 = select.where.limit.order_by.alias.select(...)
...
*Source: test_deprecations.py:127 | Complexity: Intermediate | Last updated: 2026-02-20*