# How To: Order By Selectable In Unions
...
Workflow: test order by selectable in unions
...
### Step 2: Assign s1 = select.where.order_by(...)

```python
s1 = select(table).where(table.c.id == 2).order_by(table.c.id)
```

### Step 3: Assign s2 = select.where.order_by(...)
...
*Source: test_deprecations.py:99 | Complexity: Intermediate | Last updated: 2026-02-20*